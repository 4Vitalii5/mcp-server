...
```bash
    git clone https://github.com/4Vitalii5/mcp-server
    cd mcp-graphql-server
    mvn clean install
```

2. **Запуск бази данних**:
Запустити контейнер бази данних за допомогою docker-compose.yml
```bash
...
| **SSE** | Server-Sent Events | Односпрямовані повідомлення від сервера |
...
## Що не вдалось не реалізувати

1. **Графічний клієнт на базі GraphQL** - на жаль навіть після тривалих спроб не вдалось реалізувати. Постійно отримую помилку що не можуть завантажитись стилі.
